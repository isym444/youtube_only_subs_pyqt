...
class Video:
    ...
    def __init__(self, video_id: str):
        ...
        """
        pattern = re.compile(r'(?:youtu\.be/|youtube\.com/watch\?v=|youtube\.com/embed/|^)([A-Za-z0-9_-]{11})')
        match = pattern.search(video_id)
        
        if match:
            self._matched_id = match.group(1)
        else:
            # If the pattern doesn't match, check if the input itself is an 11-character video ID
